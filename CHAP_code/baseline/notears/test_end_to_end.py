...
def convert_dataset_to_numpy(dataset_obj):
    """Convert CHAP dataset to numpy matrix."""
    X_list = []
...
def create_fixed_mask_model(model_cls, v, num_classes_dict, fixed_mask, **kwargs):
    """Create a CHAP model variant that uses a fixed causal mask instead of learning one."""
...
def main():
    ...
    # Step 3: Run CHAP with fixed NOTEARS mask vs end-to-end CHAP
    sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    ...
    logger.info('\n=== Two-stage: NOTEARS mask + CHAP predictor ===')
    ...
    logger.info('Training CHAP with NOTEARS fixed mask...')
    ...
    # ---- End-to-end: CHAP with learned mask ----
    logger.info('\n=== End-to-end: CHAP with learned mask ===')
    ...
    logger.info('Training CHAP end-to-end...')
    ...
    if metric_name == 'AUC':
        ...
        logger.info(f'CHAP (end-to-end):        AUC = {results_e2e["auc"]:.4f}')
    else:
        import math
        ...
        logger.info(f'CHAP (end-to-end):        RMSE = {rmse_e2e:.4f}')
