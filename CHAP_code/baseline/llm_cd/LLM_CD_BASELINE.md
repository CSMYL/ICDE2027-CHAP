...
and Data-Driven Reasoning** to the CHAP tabular data pipeline.
...
1. Load CHAP-format tabular data via `baseline/baseline_datasets.py`.
...
The CHAP GitHub repository documents six real-world datasets: `adult`,
`cardio`, `creditcard`, `diamonds`, `elevator`, and `housesale`. The local
workspace has newer dataset loaders than the published CHAP repository, so this
