...
# ============================================
DATASET="meps"              # 数据集名称
EPOCHS=40                    # 训练轮数（housesale样本较少，增加轮数以充分训练）
BATCH_SIZE=512                # 批次大小（保持64，样本数适中）
HIDDEN_DIM=64               # 隐藏层维度（保持128，利用分类特征连接的优势）
N_LAYERS=2                   # GNN层数（保持3层，分类特征连接通常不需要太深）
DROPOUT=0.25                 # Dropout比例（housesale有分类特征，过拟合风险较低，降低到0.25）
LR=1e-3                    # 学习率（分类特征连接更稳定，使用中等学习率）
TRAIN_SPLIT=0.8              # 训练集比例
DEVICE="cuda:2"                # 设备 (cuda 或 cpu)
